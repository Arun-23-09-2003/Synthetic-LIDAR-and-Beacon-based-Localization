...
Each filter case is run on the same scenario data, so differences between cases are due to estimator configuration rather than regenerated measurements. The cases are:
\begin{enumerate}
...
Metrics are computed by comparing the filter output to the ground truth using \texttt{compute\_filter\_metrics.m}. The position, heading, and velocity RMSE definitions are
\begin{align}
    \mathrm{RMSE}_p &=
