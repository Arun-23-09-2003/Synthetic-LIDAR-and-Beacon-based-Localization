...
(a) Scenario 1 & (b) Scenario 2 & (c) Scenario 3
\end{tabular}
\caption{Beacon placements and nominal coverage regions for the three scenarios. Beacon range limits and penetration budgets are scenario-specific configuration parameters.}
...
(a) Scenario 1 & (b) Scenario 2 & (c) Scenario 3
\end{tabular}
\caption{Ground-truth trajectories. Requested waypoints are routed through wall openings by the wall-aware planner, and trajectories are rejected if sampled segments intersect walls.}
...
\end{tabular}
\caption{Representative sensor behavior. Panel (a) shows ray-cast LiDAR returns, panel (b) shows beacon dropout and availability over time, and panel (c) shows range/RSSI behavior for selected beacons.}
