...
For each configured scenario, \path{generate_scenario_dataset.m} creates a wall-segment map, beacon definitions, a wall-validating ground-truth trajectory, noisy prediction inputs, beacon measurements, LiDAR scans, and synthetic LiDAR pose measurements. The saved data are then loaded by \texttt{run\_filter\_case.m}, which evaluates five cases using the same truth and measurement files. Finally, \texttt{collect\_results\_tables.m} computes RMSE and sensor availability metrics, and \texttt{generate\_project\_figures.m} exports the result figures used in this manuscript.
